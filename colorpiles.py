...
class ColorPile:
    ...
    def __add__(self, cmap2):
        cmap3 = ColorPile(color_map=self.color_map + cmap2.color_map, 
                         name=self.name + '+' + cmap2.name)
        return cmap3
    
    def __getitem__(self, i):
        if isinstance(i, int):
            return ColorPile(color_map=self.color_map[i:i+1],
                            name = self.name)
        elif isinstance(i, slice):
            return ColorPile(color_map=self.color_map[i],
                            name = self.name)
    ...
    def from_matplotlib(self, name='inferno', n=10):
        ...
        # Many ColorPiles do not have the .colors attribute. Don't ask why.
        ...
        return ColorPile(color_map=list(map(list, plt_map)),
                        name=name)
    ...
    def from_list(self, color_list=['red', 'black']):
        ...
        for color in color_list:
            ...
            if isinstance(color, str):
                ...
                raise ValueError(f'{color} neither in {color_dict.keys()} nor a valid RGB tuple')

        return ColorPile(color_map=colors,
                        name='from_list')
    ...
    def invert(self):
        return ColorPile(color_map=self.color_map[::-1],
                        name = self.name + '_inverted')
    ...
    def loop(self, n=1):
        ...
        return ColorPile(color_map=color_map,
                        name=name)
    ...
    def map_to_angle(self, x, y, waves=3):
        ...
        indexes = np.round(((np.sin(theta) + 1) / 2)
                           * (len(cmap)-1)).astype(int)
        # indexes = np.digitize(np.sin(theta), bins) - 1

        colors = [cmap[i] for i in indexes]

        return colors
    
    def map_to_indexes(self, indexes):
        return [self.color_map[i] for i in indexes]
        
    
    def set_name(self, name=None):
        if isinstance(name, str):
            return ColorPile(color_map=self.color_map,
                            name = name)
        else:
...
if __name__ == '__main__':
    ...
    cmap4 = ColorPile().from_list(color_list=[(255, 0, 0), 'yellow', 'blue']).extend(100)
